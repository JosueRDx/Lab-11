        ...
        <form method="POST" action="{{ route('contacts.store') }}" enctype="multipart/form-data">
            ...
            <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                ...
                <div class="sm:col-span-6">
                    ...
                    <div class="mt-2">
                        <input type="date" name="birthday" class="block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" value="{{ old('birthday') }}">
                    </div>
                </div>
                <div class="sm:col-span-6">
                    <label for="profile_picture" class="block text-md font-medium leading-6 text-gray-900">Foto de perfil</label>
                    <div class="mt-2">
                        <input type="file" name="profile_picture" class="block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">
                    </div>
                </div>
            </div>
